...
DHTMLGoodies_contextMenu.prototype = 
{
	...
	__evalUrl : function()
	{
		...
		if(js == "JSTreeObj.addItem(this,referenceToDHTMLSuiteContextMenu.srcElement)"){
		...
		for(var no in this.menuItems){	// Looping through menu items		
			...
			if(this.menuItems[no]['depth']==1){
				...
				if(this.menuItems[no]['separator']){
					...
					if(this.menuItems[no]['itemText'] == "Add"){
						li.innerHTML = '<a href="#" onclick="addNodeItem();">' + this.menuItems[no]['itemText'] + '</a>';
					}else{
