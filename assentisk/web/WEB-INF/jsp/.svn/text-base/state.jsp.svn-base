...
$("div.leftpanelinner>ul>li#group").css("background","#003366");
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('stateList.master.htm').
    success(function(data, status, headers, config) {
 
    $scope.items = data.stateBean;
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deleteState.master.htm?id="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
  ...
  <div class="mainpanel">
   <%@include file="header.jsp" %>
    ...
    <div class="pageheader">
      <h2><c:out value="${sessionScope.State_Header}"/></h2>
      <div class="breadcrumb-wrapper">
        <span class="label"><c:out value="${sessionScope.DashBoard_YouAreHere}"/>:</span>
        <ol class="breadcrumb">
         <li><a href="dashboard.login.htm">Assentisk</a></li>
          <li class="active"><c:out value="${sessionScope.State_Header}"/></li>
        </ol>
      </div>
    </div>
   ...
   <div ng-app="" ng-controller="personController">
   	<c:if test="${sessionScope.add_mod_35=='1'}">     
   <a class="btn btn-success" href="addstate.master.htm" title="<c:out value="${sessionScope.tooltip_add}"/>" style="margin-bottom:10px;"><c:out value="${sessionScope.tooltip_add}"/>&nbsp;&nbsp;<i class="fa fa-plus"></i></a>
       </c:if>
            ...
            <span class="add-on"><i class="icon-search"></i></span>
    		 ...
    		 <table class="table table-striped table-condensed table-hover" id="state">
      			  <thead>
           			 <tr>
                                		
                  		<th class="StateName" style="min-width:480px;"><a ng-click="sort_by('StateName')"><i class="icon-sort"></i><c:out value="${sessionScope.State_Name}"/></a></th>
                  		<th class="CountryName" style="min-width:450px;"><a ng-click="sort_by('CountryName')"><i class="icon-sort"></i><c:out value="${sessionScope.Country_Header}"/></a></th>
<!--                  		<th class="IsActive">IsActive&nbsp;<a ng-click="sort_by('IsActive')"><i class="icon-sort"></i></a></th>                              -->
           			     <th style="min-width:150px;"><c:out value="${sessionScope.State_Status}"/></th>                              
       ...
       <tbody>
            <tr data-ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">            		
            		<td style="min-width:480px;word-wrap:break-word;">
            				{{item.StateName}}
            		</td>
            		<td style="min-width:450px;word-wrap:break-word;">
            				{{item.CountryName}}
            		</td>
<!--            		<td>-->
...
<!--            		</td>-->
            		 ...
            		 <c:if test="${sessionScope.edit_mod_35=='1'}">  
								<a class="btn btn-info" href="editstate.master.htm?stateid={{item.StateID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i></a>
							</c:if>
							 <c:if test="${sessionScope.delete_mod_35=='1'}">  
								<button class="btn btn-danger" onclick="confirmRow({{item.StateID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>"><i class="fa fa-trash-o "></i></button>
            		 </c:if>
            		 </td>
            </tr>
         </tbody>    
