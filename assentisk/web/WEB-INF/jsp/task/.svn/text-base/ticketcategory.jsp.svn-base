...
function cityController($scope,$http,$filter) {
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('ticketcategorylist.assets.htm').
    success(function(data, status, headers, config) {
    
    $scope.items = data.ticketCatBeans;
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deleteticketcat.assets.htm?id="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
			...
			<body class="<c:out value="${sessionScope.bodyClass}"></c:out>">
					...
					<div class="mainpanel">
						...
						<div class="pageheader">
							<h2>
								<c:out value="${sessionScope.ticket_category_name}"/>
							</h2>
						...
						<div class="contentpanel">
							<div ng-app="" ng-controller="cityController">
								<c:if test="${sessionScope.add_mod_31=='1'}">
									<a class="btn btn-success" href="addticketcat.assets.htm" title="<c:out value="${sessionScope.tooltip_add}"/>"
								...
								<table class="table table-striped table-condensed table-hover"
									...
									<thead>
										<tr>
											...
											<th class="category" style="min-width:820px;max-width:820px;">
												
												<a ng-click="sort_by('category')"><i class="icon-sort"></i><c:out value="${sessionScope.ticket_cat_name}"/>
												</a>
											</th>
<!--											<th class="IsActive">-->
<!--												IsActive&nbsp;-->
...
<!--											</td>-->
											<td class="center" style="min-width:250px;max-width:250px;">
												<c:if test="${sessionScope.edit_mod_31=='1'}">
													<a class="btn btn-info"
														href="editticketcat.assets.htm?ticketid={{item.id}}"
														title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i>
													</a>
												</c:if>
												<c:if test="${sessionScope.delete_mod_31=='1'}">
