...
function personController($scope,$http,$filter) {
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('supportList.assets.htm').
    success(function(data, status, headers, config) {
    $scope.items = data.supportBeans;
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deleteticket.assets.htm?id="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
...
<%@include file="/WEB-INF/jsp/leftpanel.jsp"%>
    ...
    <div class="pageheader">
      <h2><c:out value="${sessionScope.supportTicket_Header}"/></h2>
      <div class="breadcrumb-wrapper">
        <span class="label"><c:out value="${sessionScope.DashBoard_YouAreHere}"/>:</span>
        <ol class="breadcrumb">
         <li><a href="dashboard.login.htm">Assentisk</a></li>
          <li class="active"><c:out value="${sessionScope.supportTicket_Header}"/></li>
        </ol>
      </div>
    </div>
   ...
   <div ng-app="" ng-controller="personController">
   	<c:if test="${sessionScope.add_mod_30=='1'}">
   <a class="btn btn-success" href="addticket.assets.htm" title="<c:out value="${sessionScope.tooltip_add}"/>" style="margin-bottom:10px;"><c:out value="${sessionScope.tooltip_add}"/>&nbsp;&nbsp;<i class="fa fa-plus"></i></a>
    		 ...
    		 <table class="table table-striped table-condensed table-hover" id="country">
      			  <thead>
           			 <tr>
                  		...
                  		<th class="title" style="min-width:200px;max-width:200px;"><a ng-click="sort_by('title')"><i class="icon-sort"></i><c:out value="${sessionScope.ticket_title}"/></a></th>
           			    <th class="details" style="min-width:400px;max-width:400px;"><a ng-click="sort_by('details')"><i class="icon-sort"></i><c:out value="${sessionScope.ticket_details}"/></a></th>
           			     <th class="priority" style="min-width:150px;max-width:150px;"><a ng-click="sort_by('priority')"><i class="icon-sort"></i><c:out value="${sessionScope.ticket_priority}"/></a></th>
           			     <th class="assignedTo" style="min-width:200px;max-width:200px;"><a ng-click="sort_by('assignedTo')"><i class="icon-sort"></i><c:out value="${sessionScope.ticket_assignedTo}"/></a></th>
           			     
           			     
           			     
           			     <th><c:out value="${sessionScope.Country_Action}"/></th>                              
       ...
       <tbody>
            <tr ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">            	
					...
					<td style="min-width:400px;max-width:400px;word-wrap: break-word;">
            				{{item.details}}
            		</td>
            		...
            		<td style="min-width:150px;max-width:150px;word-wrap: break-word;">
            		 ...
            		 <td class="center"  style="min-width:200px;max-width:200px;">  
            		 	<c:if test="${sessionScope.edit_mod_30=='1'}">          		 			
								<a class="btn btn-info" href="editticket.assets.htm?id={{item.id}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i></a>
								</c:if>
								<c:if test="${sessionScope.delete_mod_30=='1'}">     
