...
totalC=$("#totalCount").val();
	for(i=1;i<$("#totalCount").val();i++){
	contactName.push($("#"+i).val());
	}
						 ...
						 /*  else if(!phonefilter.test(phoneContact) && phoneContact!=''){	
							...
							else
							{
								...
								$.get("savevendorContact.org.htm?nameContact="
													...
													count=count +1;
													$("#company > tbody").append("<tr class='description_text_contact' id='contact"+count+"' ><td id='name"+count+"'><span class='spaceform' >"+nameContact+"</span></td>"
									...
									$('#company > tbody').on('click','.removeform',function(){
										
										var contactid=this.id;
									
										contactid=contactid.replace("contact", ""); 
											var name="#name"+contactid;
											var value=$(name).find('span').text();
								...
								<form ng-submit="saveData()">
										...
										<div class="col-sm-6">
											<div class="form-group">
												...
												<input type="text" id="vendorname" name="vendorname" class="form-control"
													ng-model="vendorname" ng-init='vendorname = "<c:out value="${VendorName}"/>"' maxlength="250">
								  ...
								  <div class="row">
															...
															<tr id="con<c:out value="${venlist.ID}"/>">
																...
																<td id="email_<c:out value="${venlist.email}" />">
																	<span class='spaceform'><c:out value="${venlist.email}" />
																				<span class='remove'  style="cursor: pointer;" onclick="deleteContact('<c:out value="${venlist.ID}"/>','<c:out value="${venlist.Name}" />');">X</span></span>
																</td>
															</tr>
														</c:forEach>
													</tbody>
									...
									<c:set var="countId" value="1" scope="page"/> 
										<c:forEach var="venlist" items="${vendorContact}">	
									<input type="hidden"  class="hideValue" id="<c:out value="${countId}"/>" value="<c:out value="${venlist.Name}" />"/>
									<c:set var="countId" value="${countId + 1}" scope="page"/>
									</c:forEach>
									<input type="hidden" id="totalCount" value=<c:out value="${countId}" />>
									<!-- row -->
