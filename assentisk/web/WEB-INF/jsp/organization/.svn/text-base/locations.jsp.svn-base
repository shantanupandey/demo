...
var rowID;
 var sortingOrder = 'CompanyID';
function locationsController($scope,$http,$filter) {
			...
			$("div.leftpanelinner>ul>li#organization>ul>li#locations").addClass("active");
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('locationsList.org.htm').
    success(function(data, status, headers, config) {
   
       $scope.items = data.locationsBeans;
...
locationsController.$inject = ['$scope','$http','$filter'];  
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deletelocation.org.htm?locationid="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
...
<body class="<c:out value="${sessionScope.bodyClass}"></c:out>">
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<c:out value="${sessionScope.Location_Header}"/>
				</h2>
			...
			<div class="contentpanel">
				<div ng-app="" ng-controller="locationsController">
				<c:if test="${sessionScope.add_mod_6=='1'}">
					<a class="btn btn-success" href="addlocation.org.htm"
					...
					<table class="table table-striped table-condensed table-hover"
						...
						<thead>
							<tr>
								...
								<th class="Name" style="min-width:200px;max-width:200px;">
									
									<a ng-click="sort_by('Name')"><i class="icon-sort"></i> <c:out value="${sessionScope.Location_location}"/></a>
								</th>
								
								<th class="StateName" style="min-width:150px;max-width:150px;">
									
									<a ng-click="sort_by('StateName')"><i class="icon-sort"></i><c:out value="${sessionScope.Location_State}"/>
									</a>
								</th>
								<th class="CountryName" style="min-width:150px;max-width:150px;">
									
									<a ng-click="sort_by('CountryName')"><i class="icon-sort"></i><c:out value="${sessionScope.Location_Country}"/>
									</a>
								</th>
								<th class="CityName" style="min-width:150px;max-width:150px;">
								
									<a ng-click="sort_by('CityID')"><i class="icon-sort"></i><c:out value="${sessionScope.Location_City}"/>
									</a>
								</th>
								
								
								<th class="Contactno" style="min-width:150px;max-width:150px;">
								
									<a ng-click="sort_by('Contactno')"><i class="icon-sort"></i><c:out value="${sessionScope.Location_contact}"/>
									</a>
								</th>
								
								<th class="Email1" style="min-width:150px;">
									
									<a ng-click="sort_by('Email1')"><i class="icon-sort"></i><c:out value="${sessionScope.Location_Email}"/></a>
								</th>
...
<!--								</th>-->
								<th style="min-width:150px;max-width:150px;">
									<c:out value="${sessionScope.Location_Status}"/>
								</th>
						...
						<tbody>
							<tr
								...
								<c:if test="${sessionScope.edit_mod_6=='1'}">
									<a class="btn btn-info"
										href="editlocation.org.htm?id={{item.LocationID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i
										class="fa fa-edit "></i> </a>
										</c:if>
											<c:if test="${sessionScope.delete_mod_6=='1'}">
									<button class="btn btn-danger"
										onclick="confirmRow({{item.LocationID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>">
