...
var rowID;
 var sortingOrder = 'ApplicationID';
function applicationsController($scope,$http,$filter) {

	        $("div.leftpanelinner>ul>li#applications").css("background","#003366");
    	    $("div.leftpanelinner>ul>li#applications>a").css("color","white");
    		$("div.leftpanelinner>ul>li#applications>a>span").css("color","white");
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('applicationsList.org.htm').
    success(function(data, status, headers, config) {
   
       $scope.items = data.applicationsBeans;
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deleteApplication.org.htm?id="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
...
<body class="<c:out value="${sessionScope.bodyClass}"></c:out>">
		...
		<div class="mainpanel">
			...
			<div class="pageheader">
				<h2>
					<c:out value="${sessionScope.Application_Header}"/>
				</h2>
			...
			<div class="contentpanel">
				...
				<c:choose>
					...
					<a class="btn btn-success" href="addapplication.org.htm"
					...
					<c:otherwise>
						<div class="btn btn-invisble" style="margin-bottom: 10px;">&nbsp;</div>
					</c:otherwise>
				</c:choose>
					...
					<table class="table table-striped table-condensed table-hover"
						...
						<thead>
							<tr>
								...
								<th class="ApplicationName" style="min-width:200px;max-width:270px;">
									
									<a ng-click="sort_by('ApplicationName')"><i class="icon-sort"></i><c:out value="${sessionScope.Application_Name}"/></a>
								</th>
								<th class="Name" style="min-width:200px;max-width:200px;">									
									<a ng-click="sort_by('Name')"><i class="icon-sort"></i><c:out value="${sessionScope.Application_Location}"/>
									</a>
								</th>
								<th class="LegalEntityName" style="min-width:200px;max-width:200px;">									
									<a ng-click="sort_by('LegalEntityName')"><i class="icon-sort"></i><c:out value="${sessionScope.Application_Division}"/>
									</a>
								</th>
								<th class="DeptName" style="min-width:200px;max-width:200px;">									
									<a ng-click="sort_by('DeptName')"><i class="icon-sort"></i><c:out value="${sessionScope.Application_Department}"/>
									</a>
								</th>
								<th class="EmpName" style="min-width:150px;max-width:150px;">									
									<a ng-click="sort_by('EmpName')"><i class="icon-sort"></i><c:out value="${sessionScope.Application_ResponsibleHead}"/>
									</a>
								</th>
...
<!--								</th>-->
						...
						<tbody>
							<tr
								...
								<td style="min-width:200px;max-width:270px;word-wrap:break-word;">
...
<!--								</td>-->
								...
								<c:if test="${sessionScope.edit_mod_9=='1'}">
									<a class="btn btn-info"
										href="editapplication.org.htm?id={{item.ApplicationID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i
										class="fa fa-edit "></i> </a>
										</c:if>
										<c:if test="${sessionScope.delete_mod_9=='1'}">
											<button class="btn btn-danger"
												onclick="confirmRow({{item.ApplicationID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>">
