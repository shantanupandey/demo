...
var rowID;
 var sortingOrder = 'BusinessProcessID';
function businessProcessesController($scope,$http,$filter) {
			...
			$("div.leftpanelinner>ul>li#organization>ul>li#busiprocess").addClass("active");
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('businessProcessesList.org.htm').
    success(function(data, status, headers, config) {
   
       $scope.items = data.businessProcessesBeans;
...
businessProcessesController.$inject = ['$scope','$http','$filter'];  
...
{	
	...
	if(action == 'yes')
	{
		...
		$.get("deleteBusinessProcess.org.htm?id="+rowID,function(data,status){ location.reload(true);});
	}
	else
	{
...
<body class="<c:out value="${sessionScope.bodyClass}"></c:out>">
		...
		<div class="mainpanel">
			...
			<div class="contentpanel">
				<div ng-app="" ng-controller="businessProcessesController">
				<c:if test="${sessionScope.add_mod_8=='1'}">
					<a class="btn btn-success" href="addbusinessprocess.org.htm"
					...
					<table class="table table-striped table-condensed table-hover"
						...
						<thead>
							<tr>
								...
								<th class="BusinessProcessName" style="min-width:200px;max-width:200px;">
									
									<a ng-click="sort_by('BusinessProcessName')"><i class="icon-sort"></i><c:out value="${sessionScope.BuisnessProcessName}"/></a>
								</th>
								<th class="Name" style="min-width:200px;max-width:200px;">
								
									<a ng-click="sort_by('Name')"><i class="icon-sort"></i><c:out value="${sessionScope.BuisnessProcessLocation}"/>
									</a>
								</th>
								<th class="LegalEntityName" style="min-width:200px;max-width:200px;">
									
									<a ng-click="sort_by('LegalEntityName')"><i class="icon-sort"></i><c:out value="${sessionScope.BuisnessProcessDivision}"/>
									</a>
								</th>
								<th class="DeptName" style="min-width:200px;max-width:200px;">
							
									<a ng-click="sort_by('DeptName')"><i class="icon-sort"></i>		<c:out value="${sessionScope.BuisnessProcessDepartment}"/>
									</a>
								</th>
								<th class="EmpName" style="min-width:200px;max-width:200px;">
									
									<a ng-click="sort_by('EmpName')"><i class="icon-sort"></i><c:out value="${sessionScope.BuisnessProcessResponsibleHead}"/>
									</a>
								</th>
...
<!--								</td>-->
								...
								<c:if test="${sessionScope.edit_mod_8=='1'}">
									<a class="btn btn-info"
										href="editbusinessprocess.org.htm?id={{item.BusinessProcessID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i
										class="fa fa-edit "></i> </a>
										</c:if>
										<c:if test="${sessionScope.delete_mod_8=='1'}">
									<button class="btn btn-danger"
										onclick="confirmRow({{item.BusinessProcessID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>">
