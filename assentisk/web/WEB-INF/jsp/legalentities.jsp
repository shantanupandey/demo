...
var rowID;
 var sortingOrder = 'LegalEntityID';
function legalEntitiesController($scope,$http,$filter) {
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('legalEntitiesList.org.htm').
    success(function(data, status, headers, config) {
    $scope.items = data.legalEntitiesBean;
  ...
  <div class="mainpanel">
   <%@include file="header.jsp" %>
    ...
    <div class="pageheader">
      <h2><c:out value="${sessionScope.Devison_Header}"/></h2>
      <div class="breadcrumb-wrapper">
        <span class="label"><c:out value="${sessionScope.DashBoard_YouAreHere}"/>:</span>
        <ol class="breadcrumb">
       <li><a href="dashboard.login.htm">Assentisk</a></li>
          <li class="active"><c:out value="${sessionScope.Devison_Header}"/></li>
        </ol>
      </div>
    </div>
   ...
   <c:if test="${sessionScope.add_mod_5=='1'}">
   		  <a class="btn btn-success" href="addlegalentity.org.htm" style="margin-bottom:10px;" title="<c:out value="${sessionScope.tooltip_add}"/>"><c:out value="${sessionScope.tooltip_add}"/>&nbsp;&nbsp;<i class="fa fa-plus"></i></a>
           </c:if>
            <div class="input-append">
                <input type="text" ng-model="query" ng-change="search()" class="input-large search-query" placeholder="<c:out value="${sessionScope.search}"/>">
      ...
      <div class="content demo-x">
    		 <table class="table table-striped table-condensed table-hover" id="legalentity">
      			  <thead>
           			 <tr>
                  		...
                  		<th class="LegalEntityName"  style="min-width:430px;max-width:430px;">                  		                  		 
                  		   <a ng-click="sort_by('LegalEntityName')"><i class="icon-sort"></i>  <c:out value="${sessionScope.Division_Name}"/></a>
                  		  </th>
                  		<th class="LegalEntityDescription"  style="min-width:500px;max-width:500px;"><a ng-click="sort_by('LegalEntityDescription')"><i class="icon-sort"></i><c:out value="${sessionScope.Division_Description}"/></a></th>                  	    
           			     <th style="min-width:200px;max-width:200px;"><c:out value="${sessionScope.Division_Action}"/></th>                              
       ...
       <tbody>
            <tr data-ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">
            		...
            		<td style="min-width:430px;max-width:430px;word-wrap:break-word;">
            				{{item.LegalEntityName}}
            		</td>
            		<td style="min-width:500px;max-width:500px;word-wrap:break-word;">		
<!--							<div ng-switch on="item.legalDesclen"><span ng-switch-when="true">{{item.legaldesc1data}}....</span>-->
<!--							<span ng-switch-when="false">{{item.legaldesc1data}}</span></div>		-->

							{{item.LegalEntityDescriptiondata}}
					</td>
            	 ...
            	 <c:if test="${sessionScope.edit_mod_5=='1'}">
            		            <a class="btn btn-info" href="editlegalentity.org.htm?legalentityid={{item.LegalEntityID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i></a>
           </c:if>
           	 <c:if test="${sessionScope.delete_mod_5=='1'}">
								<button class="btn btn-danger" onclick="confirmRow({{item.LegalEntityID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>"><i class="fa fa-trash-o "></i></button>
           </c:if>
