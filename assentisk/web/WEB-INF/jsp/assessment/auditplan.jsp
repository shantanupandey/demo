...
<link rel="stylesheet" href="css/jquery.mCustomScrollbar.css">
 ...
 <script type="text/javascript">
  ...
  var rowID;
 var sortingOrder = 'auditName';
function personController($scope,$http,$filter) {
			$("div.leftpanelinner>ul>li#task").css("background","#003366");
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('auditplanList.assess.htm').
    success(function(data, status, headers, config) {
    $scope.items = data.auditPlans;
...
function callHistory(id){
		...
		$.get("historyDetails.assess.htm?auditid="+id,function(data,status){ 
...
<%@include file="/WEB-INF/jsp/leftpanel.jsp"%>
    ...
    <div class="pageheader">
      <h2><c:out value="${sessionScope.Audit_Header}"/></h2>
      <div class="breadcrumb-wrapper">
        <span class="label"><c:out value="${sessionScope.DashBoard_YouAreHere}"/>:</span>
        <ol class="breadcrumb">
         <li><a href="dashboard.login.htm">Assentisk</a></li>
          <li class="active"><c:out value="${sessionScope.Audit_Header}"/></li>
        </ol>
      </div>
    </div>
   ...
   <div ng-app="" ng-controller="personController">
   	<c:if test="${sessionScope.add_mod_18=='1'}">
   <a class="btn btn-success" href="addauditplan.assess.htm" title="<c:out value="${sessionScope.tooltip_add}"/>" style="margin-bottom:10px;"><c:out value="${sessionScope.tooltip_add}"/>&nbsp;&nbsp;<i class="fa fa-plus"></i></a>
      ...
      <div class="content demo-x">
    		 <table class="table table-striped table-condensed table-hover" id="country">
      			  <thead>
           			 <tr>
                  		 ...
                  		 <th class="auditName" style=min-width:150px;max-width:150px;><a ng-click="sort_by('auditName')"><i class="icon-sort"></i><c:out value="${sessionScope.auditname}"/></a></th>
						 <th class="Name" style="min-width:150px;max-width:150px;"><a ng-click="sort_by('Name')"><i class="icon-sort"></i><c:out value="${sessionScope.auditlocation}"/></a></th>
           			     <th class="EmpName" style=min-width:150px;max-width:150px;><a ng-click="sort_by('EmpName')"><i class="icon-sort"></i><c:out value="${sessionScope.auditupdateBy}"/></a></th>
           			     <th class="ModifiedDate" style="min-width:150px;max-width:150px;"><a ng-click="sort_by('ModifiedDate')"><i class="icon-sort"></i><c:out value="${sessionScope.auditupdatedon}"/></a></th>
           			     <th class="versions" style="min-width:150px;max-width:150px;"><a ng-click="sort_by('versions')"><i class="icon-sort"></i><c:out value="${sessionScope.auditversion}"/></a></th>
           			     <th class="statusdata" style="min-width:150px;max-width:150px;"><a ng-click="sort_by('statusdata')"><i class="icon-sort"></i><c:out value="${sessionScope.auditstatus}"/></a></th>
       ...
       <tbody>
            <tr ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">            	
            		...
            		<td  style="min-width:150px;max-width:150px;word-wrap:break-word;">
            					<a  onclick="callHistory({{item.ID}})" title="History">{{item.versions}}</a>
            		...
            		<td  style="min-width:150px;max-width:150px;word-wrap:break-word;">
            		 ...
            		 <td class="center" style="min-width:150px;max-width:150px;">  
            		 	<c:if test="${sessionScope.edit_mod_18=='1'}">          		 			
								<a class="btn btn-info" href="editauditplan.assess.htm?auditid={{item.ID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i></a>
								</c:if>
						<a class="btn btn-teal" href="auditplandetails.assess.htm?auditid={{item.ID}}" title="<c:out value="${sessionScope.tooltip_editauditplandetails}"/>"><i class="fa fa-info"></i></a>
						<a class="btn btn-info" href="ganttChart.assess.htm?auditid={{item.ID}}" title="Gantt Chart"><i class="fa fa-bars"></i></a>
