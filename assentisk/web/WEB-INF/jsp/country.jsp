...
function personController($scope,$http,$filter) {
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
  	$http.get('countryList.master.htm').
    success(function(data, status, headers, config) {
    $scope.items = data.masterBean;
  ...
  <div class="mainpanel">
   <%@include file="header.jsp" %>
    ...
    <div class="pageheader">
      <h2><c:out value="${sessionScope.Country_Header}"/></h2>
      <div class="breadcrumb-wrapper">
        <span class="label"><c:out value="${sessionScope.DashBoard_YouAreHere}"/>:</span>
        <ol class="breadcrumb">
         <li><a href="dashboard.login.htm">Assentisk</a></li>
          <li class="active"><c:out value="${sessionScope.Country_Header}"/></li>
        </ol>
      </div>
    </div>
   ...
   <div ng-app="" ng-controller="personController">
   	<c:if test="${sessionScope.add_mod_34=='1'}">
   <a class="btn btn-success" href="addcountry.master.htm" title="<c:out value="${sessionScope.tooltip_add}"/>" style="margin-bottom:10px;"><c:out value="${sessionScope.tooltip_add}"/>&nbsp;&nbsp;<i class="fa fa-plus"></i></a>
    		 ...
    		 <table class="table table-striped table-condensed table-hover" id="country">
      			  <thead>
           			 <tr>
                  		...
                  		<th class="CountryName" style="min-width:800px;max-width:800px;"><a ng-click="sort_by('CountryName')"><i class="icon-sort"></i><c:out value="${sessionScope.Country_Name}"/></a></th>
<!--                  		<th class="IsActive">IsActive&nbsp;<a ng-click="sort_by('IsActive')"><i class="icon-sort"></i></a></th>                             -->
           			     <th style="min-width:250px;max-width:250px;"><c:out value="${sessionScope.Country_Action}"/></th>                              
       ...
       <tbody>
            <tr ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">            	
            		<td  style="min-width:820px;max-width:820px;word-wrap:break-word;">
            				{{item.CountryName}}
            		</td>
<!--            		<td>-->
...
<!--            			    -->
<!--            		</td>-->
            		 <td class="center" style="min-width:250px;max-width:250px;">  
            		 	<c:if test="${sessionScope.edit_mod_34=='1'}">          		 			
								<a class="btn btn-info" href="editcountry.master.htm?countryid={{item.CountryID}}" title="<c:out value="${sessionScope.tooltip_edit}"/>"><i class="fa fa-edit "></i></a>
								</c:if>
								<c:if test="${sessionScope.delete_mod_34=='1'}">     
								<button class="btn btn-danger" onclick="confirmRow({{item.CountryID}})" title="<c:out value="${sessionScope.tooltip_delete}"/>"><i class="fa fa-trash-o "></i></button>
            		 </c:if>
            		 </td>
            </tr>
         </tbody>    
